		            ...
		            <div class="nav_ulBlock">
		            	<c:forEach var="item" items="${subMenus }" begin="0" step="1" varStatus="itemStatus">
		                	<c:if test="${item.id==menu.id }">
		                		<c:choose>
									<c:when test="${fn:contains(item.link, 'http')}">  
								   		<a target="_blank" href="${item.link }" target="_blank" class="nav_liBlock current">${item.name }</a>
								   	</c:when>
								   	<c:otherwise> 
		                	...
		                	<c:if test="${item.id!=menu.id }">
			                	<c:choose>
									<c:when test="${fn:contains(item.link, 'http')}">  
								   		<a target="_blank" href="${item.link }"  target="_blank" class="nav_liBlock">${item.name }</a>
								   	</c:when>
								   	<c:otherwise> 
