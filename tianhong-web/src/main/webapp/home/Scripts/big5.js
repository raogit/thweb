...
var BodyIsFt = 0; //默认是否繁体，0-简体，1-繁体
var StranLink_Obj;
//var tmptxt;
//－－－－－－－代码开始，以下别改－－－－－－－
//转换文本
...
function StranBody(fobj) {
        if (typeof (fobj) == "object") { var obj = fobj.childNodes }
            ...
            $.get("/Ajax/AjaxCommon.aspx", { action: "setSession", num: BodyIsFt });
    ...
    function StranBodyToData(fobj) {
        if (typeof (fobj) == "object") { var obj = fobj.childNodes }
        else {
            if (BodyIsFt==1) {
                BodyIsFt = 1
                StranLink_Obj.innerHTML = StranText("简体中文", 0, 1)
                document.getElementById("StranLink").title = StranText(document.getElementById("StranLink").title, 0, 1)
            }
            else {
                BodyIsFt = 0
                StranLink_Obj.innerHTML = StranText("繁體中文", 1, 1)
                document.getElementById("StranLink").title = StranText(document.getElementById("StranLink").title, 1, 1)
            }
            var obj = document.body.childNodes
        }
...
//tmptxt = StranLink_Obj.innerHTML.toString();
    if (StranLink_Obj) {
        $.get("/Ajax/AjaxCommon.aspx", { action: "getSession" }, function(data) {
            BodyIsFt = data;
            with (StranLink_Obj) {
