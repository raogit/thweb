...
function tableData(pageNum){
	...
	var menuId = $("#menuId").val();	
	$.ajax({
        url: basePath + "/web/investment/investmentcover/list",
        type: 'post',
        dataType: 'json',
        data : {
        	menuId : menuId
...
function edit(id){
	...
	if(id<=0){
		...
		$("#pupTitle").html("修改");
		$.ajax({
	        url: basePath + "/web/investment/investmentcover/get",
	        type: 'post',
	        dataType: 'json',
	        data : {
	        	id : id
	        ...
	        success: function(data){
	        	if(data){
	        		$("#popCarrier").val(data.carrier);
	        		$("#popLegal").val(data.legal);
	        		$("#popAddress").val(data.address);
	        		$("#popEstablishTime").val(data.establishTime);
	        		
	        		$("#popMoney").val(data.money);
	        		$("#popContactName").val(data.contactName);
	        		$("#popJob").val(data.job);
	        		$("#popPhone").val(data.phone);
	        		
	        		$("#popEmail").val(data.email);
	        		$("#popBuz").val(data.buz);
	        		$("#popAchievement").val(data.achievement);
	        	}
	        }
	    });
	}
